...
inline openarm::damiao_motor::MotorType parse_motor_type(
...
struct convert<openarm_hardware::MotorConfig> {
  static bool decode(const Node& node, openarm_hardware::MotorConfig& config) {
    ...
    config.type =
        openarm_hardware::parse_motor_type(node["type"].as<std::string>());
    ...
    config.max_velocity =
        node["max_velocity"].as<double>(10.0);  // Default 10 rad/s
    return true;
  }
};
...
struct convert<openarm_hardware::GripperConfig> {
  static bool decode(const Node& node,
                     openarm_hardware::GripperConfig& config) {
    config.name = node["name"].as<std::string>();
    config.motor_type = openarm_hardware::parse_motor_type(
    ...
    config.max_velocity =
        node["max_velocity"].as<double>(5.0);  // Default 5 rad/s
    return true;
  }
};
