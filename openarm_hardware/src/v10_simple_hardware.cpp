...
hardware_interface::return_type OpenArm_v10HW::write(
  ...
  for (size_t i = 0; i < config_.arm_joints.size(); ++i) {
    ...
    const auto& motor_limits =
        openarm::damiao_motor::MOTOR_LIMIT_PARAMS[static_cast<size_t>(c.type)];
    double tau_limit = motor_limits.tMax;

    if (std::abs(tau_clamped) > tau_limit) {
      static rclcpp::Clock steady_clock(RCL_STEADY_TIME);
      RCLCPP_WARN_THROTTLE(rclcpp::get_logger("OpenArm_v10HW"), steady_clock,
                           1000,
                           "Joint %s torque command %.3f Nm exceeds motor "
                           "limit %.1f Nm - clamping!",
